...
function getRoute(event) {
  let p = event.path || "";
  if (p.includes("/api/")) return p.split("/api/")[1].replace(/^\/+/, "");
  if (p.includes("/.netlify/functions/api/")) return p.split("/.netlify/functions/api/")[1].replace(/^\/+/, "");
  return "";
}
...
exports.handler = async (event) => {
  try {
    ...
    if (method === "GET" && route === "config") {
      return response(200, { ok: true, dolls: db.dolls.map(publicDoll) });
    }
    ...
    if (method === "POST" && route === "verify") {
      ...
      if (!found) return response(404, { ok: false, error: "兑换码不存在" });
      if (remaining(found) <= 0) return response(400, { ok: false, error: "兑换码次数已用完" });
      return response(200, { ok: true, code: found.code, remaining: remaining(found) });
    }
    ...
    if (!adminOnly(event)) return response(401, { ok: false, error: "后台密码错误" });

    if (method === "GET" && route === "admin/codes") {
      return response(200, { ok: true, codes: db.codes.map(c => ({ ...c, remaining: remaining(c) })) });
    }
    ...
    if (method === "POST" && route === "admin/codes") {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const item = { code, total, used: 0, pay, createdAt: now() };
        db.codes.unshift(item);
        made.push({ ...item, remaining: total });
      }
      await saveDb(store, db);
      return response(200, { ok: true, codes: made });
    }

    if (method === "GET" && route === "admin/records") {
      return response(200, { ok: true, records: db.records });
    }

    const markMatch = route.match(/^admin\/records\/(.+)\/status$/);
    if (method === "POST" && markMatch) {
      const record = db.records.find(r => r.id === markMatch[1]);
      if (!record) return response(404, { ok: false, error: "记录不存在" });
      const body = JSON.parse(event.body || "{}");
    ...
    if (method === "GET" && route === "admin/export") {
      const rows = [["时间", "兑换码", "抓中娃娃", "对应奖励", "状态"], ...db.records.map(r => [r.time, r.code, r.doll, r.reward, r.status])];
    ...
    if (method === "POST" && stockMatch) {
      ...
      if (!doll) return response(404, { ok: false, error: "娃娃不存在" });
      doll.stock = Math.max(0, Number(body.stock || 0));
      await saveDb(store, db);
      return response(200, { ok: true, doll });
    }
    ...
    return response(404, { ok: false, error: "接口不存在", route });
  } catch (error) {
    return response(500, { ok: false, error: error.message || "服务器错误" });
  }
};
