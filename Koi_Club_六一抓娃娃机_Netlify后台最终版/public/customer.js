...
function renderDolls() {
  ...
  dolls.forEach((d, i) => {
    ...
    el.innerHTML = `
      ...
      <span>${d.unlimited ? "无限库存" : isAvailable ? "库存" + d.stock : "已抓完"}</span>
    `;
...
function renderRewards() {
  rewardList.innerHTML = dolls.map(d => `
    <div class="reward-card">
      <div class="reward-icon">${d.icon}</div>
      <div class="reward-text">${d.reward}</div>
      <div class="reward-stock">${d.unlimited ? "无限库存" : "库存：" + d.stock}</div>
    </div>
  `).join("");
}
